...
- **Server Environment:** Python and FastAPI backend hosted on a standard cloud-based server capable of handling a high volume of traffic.
...
- **Data:** Logic must follow existing university rules and must not allow students to register for plans that conflict with those rules.
