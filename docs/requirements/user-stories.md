User Stories - PathToGrab Course Planning System

1. Purpose
   
   This document describes the main user stories for PathToGrad.
   ...
   The system also supports fallback planning mode. If the LLM provider is unavailable, the rule-based planning tools can still generate a valid plan.
   
2. Skateholders
   
   Student are primary users. They need support in planning semesters, checking prerequisite, and checking graduation progress.
   Academic advisors are supporting users. They may use the system to understand student plan and give academic advice.
...
3. User Story Map
   ...
   If the LLM provider is unavailable, fallback planning mode returns a rule-based plan and template-based explanation.
...
4. User Stories
   ...
   As a student, I want to ask the LLM Plannning Agent for a semester plan, so that I can receive a suitable and explainable recommenndation.
   Acceptance Criteria:
   - The student can send a planning request.
   - The LLM Planninng Agent processes the request
   - The agent calls internal tools berfore giving the final answer.
   ...
   As an academic advisor, I want the system to detect academic risks, so that I can avoid invalid course choices or delayed graduation.
   Acceptance Criteria:
   ...
   As a student, I want the system to provide fallback planning mode, so that I can still receive a valid course plan when the LLM provider is uvailable.
   Acceptance Criteria:
   - The system detects LLM provider errors or uvavailble responses.
